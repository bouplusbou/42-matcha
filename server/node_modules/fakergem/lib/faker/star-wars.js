'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }();

function _toConsumableArray(arr) { if (Array.isArray(arr)) { for (var i = 0, arr2 = Array(arr.length); i < arr.length; i++) { arr2[i] = arr[i]; } return arr2; } else { return Array.from(arr); } }

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

var data = require('../../data/star-wars.json');

function capitalize(text) {
  return text.replace(/\b\w/, function (x) {
    return x.toUpperCase();
  });
}

var StarWars = function () {
  function StarWars(faker) {
    _classCallCheck(this, StarWars);

    this.faker = faker;
  }

  _createClass(StarWars, [{
    key: 'character',
    value: function character() {
      return this.faker.Random.element(data['characters']);
    }
  }, {
    key: 'droid',
    value: function droid() {
      return this.faker.Random.element(data['droids']);
    }
  }, {
    key: 'planet',
    value: function planet() {
      return this.faker.Random.element(data['planets']);
    }
  }, {
    key: 'quote',
    value: function quote() {
      return this.faker.Random.element(data['quotes']);
    }
  }, {
    key: 'specie',
    value: function specie() {
      return this.faker.Random.element(data['species']);
    }
  }, {
    key: 'vehicle',
    value: function vehicle() {
      return this.faker.Random.element(data['vehicles']);
    }
  }, {
    key: 'wookieSentence',
    value: function wookieSentence() {
      var _this = this;

      return capitalize([].concat(_toConsumableArray(Array(this.faker.Number.between(1, 10)).keys())).map(function (_) {
        return _this.faker.Random.element(data['wookieWords']);
      }).join(' ') + this.faker.Random.element(['.', '?', '!']));
    }
  }]);

  return StarWars;
}();

exports.default = StarWars;